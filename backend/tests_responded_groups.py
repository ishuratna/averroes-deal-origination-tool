...
chk("a stale Ready-to-assign stamp (v3) means nothing now -> Nurture", {"assignment_ready_at": "2026-08-21T10:00:00"}, "nurture")

print()
print("── One flow, two calls: Ishu -> associates (Thursday) -> partners (Monday); no fit/size fork ──")
chk("passed to partners (A), unconfirmed -> Monday list", {"track": "A"}, "partner_review")
chk("passed to partners (A), confirmed to Bea -> With Bea", {"track": "A", "owner": "Bea"}, "partner_assigned")
chk("passed to partners (A), an associate as owner is still unconfirmed", {"track": "A", "owner": "Issam"}, "partner_review")
chk("passed to associates (B), nobody has taken it -> Thursday list", {"track": "B"}, "assoc_review")
chk("passed to associates (B), Issam took it -> with associates", {"track": "B", "owner": "Issam"}, "assoc_pending")
chk("passed to associates (B), Marianna took it -> with associates", {"track": "B", "owner": "Marianna"}, "assoc_pending")
chk("passed to associates (B), Bea as owner is NOT an associate taking it", {"track": "B", "owner": "Bea"}, "assoc_review")
...
print()
print("── Talk later: asleep 6 months, then wakes back into Nurture ──")
...
chk("parked 6+ months ago -> wakes into Nurture", {"track": "later", "triaged_at": expired}, "nurture")
chk("exactly at the boundary -> awake (>= is the rule)", {"track": "later", "triaged_at": boundary}, "nurture")
...
chk("parked with NO timestamp -> awake, never lost", {"track": "later", "triaged_at": None}, "nurture")
chk("parked with empty timestamp -> awake", {"track": "later", "triaged_at": ""}, "nurture")
