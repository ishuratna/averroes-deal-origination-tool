...
TOOL_UPDATES = [
    ("2026-09-09", "Investor email rewritten (v2): an invitation to talk rather than a pitch. It introduces Averroes as a London technology investor taking growth equity or majority positions in UK and Irish software, explains that we invest deal by deal alongside a pool of investors many of whom have backed us more than once and that we are widening that circle, describes the collaborative way we work with management (Glowday and Journey named), and asks for a short call or offers a short note first. One personal line per investor is written from what we hold about them; the placeholders are gone."),
    ("2026-09-09", "Investor priority for the co-investment raise: every investor now carries a Priority tier (A / B / C / Parked) and score built for deal-by-deal cheques of GBP 250K-2M - co-invest appetite, ticket fit, software affinity, home geography (UK/IE and the GCC first, then Europe), recency, and a warm-path boost from network tags (GCC, Bea, Partner, Co-investor...) and portfolio overlap. Tier A = fits and contactable; B = fits but needs InvestorFill for a contact. Tags can be set on the investor card or stamped on a whole upload (e.g. the KSA/GCC PitchBook export or Bea's list), and both investor pages have a GCC quick filter and tier filter; the card shows the full breakdown with reasons."),
